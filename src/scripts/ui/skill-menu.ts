...
export class SkillMenu {
     ...
     */
    createSkillCard(id: string, name: string, description: string, effects: Array<{name: string, id: string, value: string}>, level: number = 0, locked: boolean = false, unlockLevel: number = 0): void {
    ...
    initializeUpgradeButtons(): void {
        // Auto Attack upgrade
        const autoAttackButton = document.getElementById('auto-attack-upgrade') as HTMLButtonElement;
        if (autoAttackButton) {
        ...
        // Blood Drain upgrade
        const bloodDrainButton = document.getElementById('blood-drain-upgrade') as HTMLButtonElement;
        if (bloodDrainButton) {
        ...
        // Bat Swarm upgrade
        const batSwarmButton = document.getElementById('bat-swarm-upgrade') as HTMLButtonElement;
        if (batSwarmButton) {
        ...
        // Shadow Dash upgrade
        const shadowDashButton = document.getElementById('shadow-dash-upgrade') as HTMLButtonElement;
        if (shadowDashButton) {
        ...
        const bloodLanceButton = document.getElementById('blood-lance-upgrade') as HTMLButtonElement;
        if (bloodLanceButton) {
        ...
        const nightShieldButton = document.getElementById('night-shield-upgrade') as HTMLButtonElement;
    ...
    update(): void {
        ...
        if (damageElement) damageElement.textContent = player.autoAttack.damage.toString();
        
        const cooldownElement = document.getElementById('auto-attack-cooldown');
        if (cooldownElement) cooldownElement.textContent = (player.autoAttack.cooldown / 1000) + 's';
        
        const rangeElement = document.getElementById('auto-attack-range');
        if (rangeElement) rangeElement.textContent = player.autoAttack.range.toString();
        ...
        if (bloodDrain) {
            ...
            if (drainDamageElement) drainDamageElement.textContent = bloodDrain.getScaledDamage().toString();
            
            const drainHealingElement = document.getElementById('blood-drain-healing');
            if (drainHealingElement) drainHealingElement.textContent = bloodDrain.getScaledHealing().toString();
            
            const drainRangeElement = document.getElementById('blood-drain-range');
            if (drainRangeElement) drainRangeElement.textContent = bloodDrain.getScaledRange().toString();
            
            const drainDurationElement = document.getElementById('blood-drain-duration');
            if (drainDurationElement) drainDurationElement.textContent = (bloodDrain.duration / 1000) + 's';
        }
        ...
        if (batSwarm) {
            ...
            if (batDamageElement) batDamageElement.textContent = batSwarm.getScaledDamage().toString();
            
            const batCountElement = document.getElementById('bat-swarm-count');
            if (batCountElement) batCountElement.textContent = batSwarm.getScaledCount().toString();
            
            const batCooldownElement = document.getElementById('bat-swarm-cooldown');
            if (batCooldownElement) batCooldownElement.textContent = (batSwarm.cooldown / 1000) + 's';
        }
        ...
        if (shadowDash) {
            ...
            if (dashDamageElement) dashDamageElement.textContent = shadowDash.getScaledDamage().toString();
            
            const dashDistanceElement = document.getElementById('shadow-dash-distance');
            if (dashDistanceElement) dashDistanceElement.textContent = shadowDash.getScaledDistance().toString();
            
            const dashInvulnElement = document.getElementById('shadow-dash-invuln');
            if (dashInvulnElement) dashInvulnElement.textContent = (shadowDash.getScaledInvulnerabilityTime() / 1000) + 's';
        }
        ...
        if (bloodLance) {
            ...
            if (lanceElement) lanceElement.textContent = bloodLance.getScaledDamage().toString();
            
            const lancePierceElement = document.getElementById('blood-lance-pierce');
            if (lancePierceElement) lancePierceElement.textContent = bloodLance.getScaledPierce().toString();
            
            const lanceHealingElement = document.getElementById('blood-lance-healing');
            if (lanceHealingElement) lanceHealingElement.textContent = bloodLance.getScaledHealing().toString();
        }
        ...
        if (nightShield) {
            ...
            const shieldAmountElement = document.getElementById('night-shield-amount');
            if (shieldAmountElement) shieldAmountElement.textContent = nightShield.getScaledShieldAmount().toString();
            
            const shieldDurationElement = document.getElementById('night-shield-duration');
            if (shieldDurationElement) shieldDurationElement.textContent = (nightShield.duration / 1000) + 's';
            
            const shieldExplosionElement = document.getElementById('night-shield-explosion');
            if (shieldExplosionElement) shieldExplosionElement.textContent = nightShield.getScaledExplosionDamage().toString();
        }
    ...
    updateUpgradeButtonStates(): void {
        ...
        const autoAttackButton = document.getElementById('auto-attack-upgrade') as HTMLButtonElement;
        if (autoAttackButton) {
            autoAttackButton.disabled = (player.skillPoints < 1 || player.autoAttack.level >= player.autoAttack.maxLevel);
        }
        ...
        if (bloodDrainButton) {
            const bloodDrain = abilities.get('bloodDrain');
            bloodDrainButton.disabled = (player.skillPoints < 1 || bloodDrain.level >= bloodDrain.maxLevel);
        }
        ...
        if (batSwarmButton) {
            const batSwarm = abilities.get('batSwarm');
            batSwarmButton.disabled = (player.skillPoints < 1 || batSwarm.level >= batSwarm.maxLevel);
        }
        ...
        if (shadowDashButton) {
            const shadowDash = abilities.get('shadowDash');
            shadowDashButton.disabled = (player.skillPoints < 1 || shadowDash.level >= shadowDash.maxLevel);
        }
    ...
    updateUnlockableAbilityButtons(): void {
        ...
        const bloodLanceButton = document.getElementById('blood-lance-upgrade') as HTMLButtonElement;
        const bloodLanceLockedElement = document.getElementById('blood-lance-locked');
        ...
        if (bloodLanceButton && bloodLanceLockedElement) {
            if (player.level < bloodLanceUnlockLevel) {
                ...
                bloodLanceButton.disabled = (player.skillPoints < 1 || bloodLance.level >= bloodLance.maxLevel);
        ...
        const nightShieldButton = document.getElementById('night-shield-upgrade') as HTMLButtonElement;
        const nightShieldLockedElement = document.getElementById('night-shield-locked');
        ...
        if (nightShieldButton && nightShieldLockedElement) {
            if (player.level < nightShieldUnlockLevel) {
                ...
                // Already unlocked, check if can upgrade
                nightShieldButton.disabled = (player.skillPoints < 1 || nightShield.level >= nightShield.maxLevel);
