...
import { SkillMenu } from './skill-menu.js';
...
export class UIManager {
    ...
    subscribeToEvents(): void {
        ...
        // Player events
        GameEvents.on(EVENTS.PLAYER_SKILL_POINT, () => this.updateSkillPointsDisplay());
    }
    ...
    updateAutoAttackToggle(): void {
        if (this.autoAttackToggle) {
            this.autoAttackToggle.textContent = `Auto-Attack: ${this.player.autoAttack.enabled ? 'ON' : 'OFF'}`;
            this.autoAttackToggle.classList.toggle('active', this.player.autoAttack.enabled);
        }
    }
