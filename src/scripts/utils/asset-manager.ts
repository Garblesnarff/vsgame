import { GameEvents } from './event-system.js';
...
export class AssetManager {
        ...
        return this.fontsToLoad.map(({ fontFamily, url }) => {
            return new Promise<FontFace>((resolve, reject) => {
                ...
                fontFace.load().then(loadedFace => {
                    ...
                    reject(new Error(`Failed to load font: ${fontFamily} - ${error.message}`));
                });
            });
        });
    }
     ...
     */
    playSound(key: string, volume: number = 1.0, loop: boolean = false): HTMLAudioElement | undefined {
