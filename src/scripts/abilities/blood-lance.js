...
export class BloodLance extends Ability {
     ...
     */
    use() {
        if (!this.unlocked || !super.use()) return false;
     ...
     * @returns {Object} - Target information { angle, targetX, targetY }
     */
    findTarget() {
        let targetX, targetY;
    ...
    findClosestEnemy() {
        ...
        if (!this.player.game || !this.player.game.enemies || this.player.game.enemies.length === 0) {
            return null;
        }
     ...
     * @returns {number} - Scaled pierce count
     */
    getScaledPierce() {
